...
export default function Dietplan() {
  ...
  const [waterDrankData, setWaterDrankData] = useState({
    labels: ['Calories Drank', 'Remaining Calories'],
    datasets: [{
      data: [0, 2000],
  ...
  const fetchMealPlan = async () => {
    try {
      ...
      if (cachedMealPlan) {
        ...
        // Cache the fetched meal plan data
        localStorage.setItem('mealPlan', JSON.stringify(mealPlanData));

        // Cache meal details (e.g., images) separately
  ...
  // Define cacheMealDetails function
  const cacheMealDetails = (mealPlanData) => {
    mealPlanData.meals.forEach((meal) => {
      // Check if meal image URL is already cached and not expired
      ...
      if (!cachedMealImage || JSON.parse(cachedMealImage).expiry < Date.now()) {
        // If not cached or expired, fetch a new image URL
        fetchMealImage(meal.id, meal.imageType);
      }
    });
  };

  // Define fetchMealImage function to fetch and cache meal image URL
  const fetchMealImage = async (mealId, imageType) => {
    try {
      ...
      const mealImageData = {
        ...
        expiry: Date.now() + 24 * 60 * 60 * 1000 // Set expiry time to 24 hours from now
      };

      // Cache the fetched meal image URL
  ...
  // Define markMealAsEaten function
  const markMealAsEaten = (index, nutrients) => {
    if (mealData && index >= 0 && index < mealData.meals.length) {
      ...
      if (waterBox) {
        waterBox.style.backgroundColor = '#36A2EB';
      }

      const eatenMeal = updatedMeals[index];

      // Update nutrition data for this meal only
      ...
      // Merge the updated nutrients with the existing data
      ...
      // Update the state with the new nutrition data
      setNutritionData(newNutritionData);
    }
  };

  // Define updateNutritionData function
  const updateNutritionData = (nutrients) => {
    setNutritionData({
      ...nutritionData,
      datasets: [{
        ...nutritionData.datasets[0],
